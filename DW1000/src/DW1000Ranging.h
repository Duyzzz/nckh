...
public:
	...
	// Custom data API
	static void setCustomData(byte* data, uint8_t len);
	static byte* getReceivedCustomData();
	static uint8_t getReceivedCustomDataLen();
	static void attachCustomDataReceived(void (*handler)(void));

	// RF timing API - returns calculated air time in microseconds based on frame length
	static uint32_t getPollAirTimeUs();
	static uint32_t getRangeReportAirTimeUs();
...
private:
	...
	static void (* _handleCustomDataReceived)(void);

	// Custom data buffers (max 50 bytes)
	static byte _customDataTX[50];
	static uint8_t _customDataTXLen;
	static byte _customDataRX[50];
	static uint8_t _customDataRXLen;

	// RF timing - frame lengths for air time calculation
	static uint8_t _pollFrameLen;
	static uint8_t _rangeReportFrameLen;

	// Calculate air time based on frame length (for MODE_LONGDATA_RANGE_LOWPOWER)
	static uint32_t calculateAirTimeUs(uint8_t frameLen);
