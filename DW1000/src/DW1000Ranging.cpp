...
void (* DW1000RangingClass::_handleCustomDataReceived)(void) = 0;

// Custom data buffers
byte DW1000RangingClass::_customDataTX[50];
uint8_t DW1000RangingClass::_customDataTXLen = 0;
byte DW1000RangingClass::_customDataRX[50];
uint8_t DW1000RangingClass::_customDataRXLen = 0;

// RF timing - frame lengths
uint8_t DW1000RangingClass::_pollFrameLen = 0;
uint8_t DW1000RangingClass::_rangeReportFrameLen = 0;
...
void DW1000RangingClass::setCustomData(byte* data, uint8_t len) {
	_customDataTXLen = (len > 50) ? 50 : len;
	memcpy(_customDataTX, data, _customDataTXLen);
}

byte* DW1000RangingClass::getReceivedCustomData() { return _customDataRX; }

uint8_t DW1000RangingClass::getReceivedCustomDataLen() { return _customDataRXLen; }

void DW1000RangingClass::attachCustomDataReceived(void (*handler)(void)) {
	_handleCustomDataReceived = handler;
}
...
uint32_t DW1000RangingClass::getPollAirTimeUs() {
	return calculateAirTimeUs(_pollFrameLen);
}

uint32_t DW1000RangingClass::getRangeReportAirTimeUs() {
	return calculateAirTimeUs(_rangeReportFrameLen);
}
		...
		if(messageType == BLINK && _type == ANCHOR) {
			...
			if(_type == ANCHOR) {
				...
				if(messageType == POLL) {
					...
					// Extract custom data from POLL
					uint8_t customOffset = SHORT_MAC_LEN + 2 + numberDevices*4;
					_customDataRXLen = data[customOffset];
					if (_customDataRXLen > 0 && _customDataRXLen <= 50) {
						memcpy(_customDataRX, data + customOffset + 1, _customDataRXLen);
						if (_handleCustomDataReceived != 0) {
							(*_handleCustomDataReceived)();
						}
					}
				...
				if(messageType == POLL_ACK) {
					...
					// Extract custom data from RANGE_REPORT
					_customDataRXLen = data[SHORT_MAC_LEN + 9];
					if (_customDataRXLen > 0 && _customDataRXLen <= 50) {
						memcpy(_customDataRX, data + SHORT_MAC_LEN + 10, _customDataRXLen);
						if (_handleCustomDataReceived != 0) {
							(*_handleCustomDataReceived)();
						}
					}
...
void DW1000RangingClass::transmitPoll(DW1000Device* myDistantDevice) {
	...
	if(myDistantDevice == nullptr) {
		...
		// Append custom data after device list
		uint8_t customOffset = SHORT_MAC_LEN + 2 + 4*_networkDevicesNumber;
		data[customOffset] = _customDataTXLen;
		if (_customDataTXLen > 0) {
			memcpy(data + customOffset + 1, _customDataTX, _customDataTXLen);
		}
	...
	// Store frame length for air time calculation
	// POLL frame: MAC + type + numDevices + (4 bytes per device) + customLen + customData
	if (myDistantDevice == nullptr) {
		_pollFrameLen = SHORT_MAC_LEN + 2 + 4*_networkDevicesNumber + 1 + _customDataTXLen;
	} else {
		_pollFrameLen = SHORT_MAC_LEN + 2 + 4; // single device, no custom data in this path
	}
	transmit(data);
}
...
void DW1000RangingClass::transmitRangeReport(DW1000Device* myDistantDevice) {
	...
	// Append custom data
	data[SHORT_MAC_LEN + 9] = _customDataTXLen;
	if (_customDataTXLen > 0) {
		memcpy(data + SHORT_MAC_LEN + 10, _customDataTX, _customDataTXLen);
	}

	copyShortAddress(_lastSentToShortAddress, myDistantDevice->getByteShortAddress());

	// Store frame length for air time calculation
	// RANGE_REPORT: MAC + type + range(4) + rxpower(4) + customLen + customData
	_rangeReportFrameLen = SHORT_MAC_LEN + 1 + 4 + 4 + 1 + _customDataTXLen;
